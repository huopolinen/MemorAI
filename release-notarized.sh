...
echo "[4/7] Embedding CTranscribe.framework (GigaAM runtime)…"
embed_ctranscribe "$APP"
report_archs "$APP"

echo "[5/7] Signing with Developer ID (inside-out, hardened runtime + timestamp)…"
# No --deep: it is deprecated for distribution and would sign the nested
# framework with the app's entitlements. sign_bundle signs the framework first
# and the app second, which is what the notary service and Gatekeeper expect.
sign_bundle "$APP" "$SIGN_IDENTITY" --timestamp
